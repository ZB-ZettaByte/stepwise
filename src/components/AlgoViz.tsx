  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
            ...
            >
              <div
                className={`absolute inset-y-0 -left-0.5 w-[5px] z-20 ${
                  sidebar.isDragging ? 'bg-amber-400/50' : 'hover:bg-slate-200'
                } transition-colors`}
              />
        ...
        {isMobile && (
          <>
            ...
            />
            <aside
              className={`fixed top-0 left-0 bottom-0 w-[280px] bg-white z-50 border-r border-slate-200 transition-transform duration-300 ease-in-out ${
            ...
            >
              <div className="h-full flex flex-col">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
                  <span className="text-sm font-semibold text-slate-950 font-heading">
                    {t.mobileMenuTitle}
                  </span>
                  <button
                    onClick={() => setMobileSidebarOpen(false)}
                    className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-slate-100 text-slate-500 hover:text-slate-950 transition-colors"
          ...
          <div className="flex-1 flex flex-col p-4 md:p-8 overflow-auto bg-white">
            {selectedAlgorithm?.acceptsCustomArray && (
              <ArrayInput
                ...
                t={t}
                askForTarget={selectedAlgorithm.category === 'Searching'}
                defaultTarget={
                  typeof currentStepData?.variables?.target === 'number'
                    ? currentStepData.variables.target
                    : undefined
                }
                onVisualize={(values, target) =>
                  applyCustomArray(
                    selectedAlgorithm.category === 'Searching' ? { array: values, target } : values,
                  )
                }
                onReset={() => applyCustomArray(undefined)}
              />
            )}
            {renderVisualization()}
          </div>
            ...
            >
              <div
                className={`absolute inset-y-0 -right-0.5 w-[5px] z-20 ${
                  codePanel.isDragging ? 'bg-amber-400/50' : 'hover:bg-slate-200'
                } transition-colors`}
              />
              <div className="h-full bg-slate-200" />
            </div>
              ...
              >
                {selectedAlgorithm ? (
                  <CodePanel
                    ...
                    cppCode={selectedAlgorithm.cppCode}
        ...
        {isMobile && (
          <>
            ...
            />
            <aside
              className={`fixed top-0 right-0 bottom-0 w-[min(360px,90vw)] bg-white z-50 border-l border-slate-200 transition-transform duration-300 ease-in-out ${
      ...
      {isMobile && selectedAlgorithm && (
        <div className="shrink-0 flex items-center justify-between px-3 py-2 border-t border-slate-200 bg-white z-10 gap-2">
          <button
            onClick={() => setMobileSidebarOpen(true)}
            className="w-8 h-8 flex items-center justify-center rounded-md hover:bg-slate-100 text-slate-500 hover:text-slate-950 transition-colors shrink-0"
          ...
          <div className="flex-1 flex items-center justify-center min-w-0">
            <div className="flex items-center gap-1" role="group" aria-label={t.controlsLabel}>
              <button
                ...
                className="p-1.5 rounded-md hover:bg-slate-100 disabled:opacity-20 text-slate-500 hover:text-slate-950 transition-all active:scale-95"
              ...
              <button
                onClick={togglePlay}
                className="w-8 h-8 rounded-full bg-slate-950 hover:bg-slate-800 flex items-center justify-center transition-all active:scale-95"
            ...
            <span className="text-[11px] text-slate-500 font-mono tabular-nums ml-2">
