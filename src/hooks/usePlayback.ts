...
import type { Algorithm, AlgorithmInput, Step } from '@lib/types'
...
export function usePlayback(locale: Locale, initialAlgorithm?: Algorithm | null) {
  ...
  const [steps, setSteps] = useState<Step[]>(() =>
    initialAlgorithm ? initialAlgorithm.generateSteps(locale) : [],
  )
  ...
  // Re-run the current algorithm with custom input (or undefined to fall back
  // to the algorithm's default example).
  const applyCustomArray = useCallback(
    (input?: AlgorithmInput) => {
      if (!selectedAlgorithm) return
      clearAutoplayTimer()
      setIsPlaying(false)
      const newSteps = selectedAlgorithm.generateSteps(locale, input)
      setSteps(newSteps)
      setCurrentStep(0)
    },
