...
def merge(left, right):
    result = []
    # Repeatedly take the smaller front item
...
export const queue: Algorithm = {
  ...
  code: `class Queue:
    ...
    def dequeue(self):
        # pop(0) removes the oldest front item
...
Classic problems:
  ...
  cppCode: getCppCode('two-pointers'),
...
export const slidingWindow: Algorithm = {
  ...
  code: `def longest_unique_substring(s):
    ...
    for end in range(len(s)):
        # Character already in window, shrink left
        while s[end] in seen:
            seen.remove(s[start])
            start = start + 1
        seen.add(s[end])
...
Classic problems:
  ...
  cppCode: getCppCode('sliding-window'),
...
def reversed_list(arr):
    # Copying the list allocates n new elements
    copy = list(arr)
    copy.reverse()
    return copy
...
Examples:
  ...
  cppCode: getCppCode('space-complexity'),
...
export const worstCaseAnalysis: Algorithm = {
  id: 'worst-case-analysis',
  name: 'Worst-case Analysis',
  category: 'Concepts',
  difficulty: 'intermediate',
  visualization: 'concept',
  code: `def quicksort_bad_case(array):
    # Bad pivot choice: always pick the last item
    if len(array) <= 1:
        return array
...
def analyze_worst_case(n):
    # If the input is already sorted and pivot is always last:
    # partitions are size 0 and n - 1 every time.
    comparisons = 0
    while n > 1:
        comparisons += n - 1
        n -= 1
    return comparisons`,
  description: `Worst-case Analysis

Worst-case analysis asks: what is the maximum cost this algorithm can take over all inputs of size n?

This is different from simply naming Big O. Big O is notation; worst-case analysis is the reasoning that identifies the adversarial input and proves an upper bound.

Example: Quicksort
  - Average case: O(n log n)
  - Worst case: O(n²)
  - The worst case happens when each pivot creates a maximally unbalanced partition

Why it matters:
  - It gives performance guarantees
  - It protects against adversarial inputs
  - It explains why randomized algorithms are useful
  - It helps choose algorithms for safety-critical systems

Related cases:
  - Best case: minimum cost
  - Average case: expected cost over a distribution
  - Worst case: guaranteed maximum cost`,
  cppCode: getCppCode('worst-case-analysis'),

  generateSteps(locale = 'en') {
    const steps: Step[] = []
    const curves = ['O(n)', 'O(n log n)', 'O(n²)']

    steps.push({
      concept: {
        type: 'bigO',
        curves: makeCurves(['O(n)', 'O(n log n)'], 'O(n log n)'),
        maxN: 10,
        yLabel: 'comparisons',
      },
      description: d(
        locale,
        'Average-case quicksort looks like O(n log n): partitions are reasonably balanced.',
        'El caso promedio de quicksort parece O(n log n): las particiones están razonablemente balanceadas.',
      ),
      codeLine: 1,
      variables: { case: 'average', partitions: 'balanced', bound: 'O(n log n)' },
    })

    steps.push({
      concept: {
        type: 'bigO',
        curves: makeCurves(curves, 'O(n²)'),
        maxN: 10,
        yLabel: 'comparisons',
      },
      description: d(
        locale,
        'Worst-case analysis looks for the input that hurts most. Sorted input plus “last item pivot” creates 0 and n-1 partitions.',
        'El análisis de peor caso busca la entrada que más duele. Entrada ordenada con pivote final crea particiones 0 y n-1.',
      ),
      codeLine: 6,
      variables: { input: 'sorted', pivot: 'last item', split: '0 and n-1' },
    })
    ...
    steps.push({
      ...
      description: d(
        locale,
        'The recurrence becomes T(n) = T(n-1) + O(n), which sums to O(n²).',
        'La recurrencia se vuelve T(n) = T(n-1) + O(n), que suma O(n²).',
      ),
      codeLine: 19,
      variables: { recurrence: 'T(n)=T(n-1)+n', sum: 'n+(n-1)+...+1', bound: 'O(n²)' },
    })

    steps.push({
      concept: {
        type: 'bigO',
        curves: makeCurves(['O(n)', 'O(n log n)', 'O(n²)'], 'O(n log n)'),
        maxN: 20,
        yLabel: 'comparisons',
      },
      description: d(
        locale,
        'Randomized pivoting makes the bad input hard to force, improving expected performance while the theoretical worst case remains O(n²).',
        'Elegir pivotes aleatorios dificulta forzar la mala entrada, mejorando el rendimiento esperado aunque el peor caso teórico siga siendo O(n²).',
      ),
      codeLine: 1,
      variables: { technique: 'randomized pivot', expected: 'O(n log n)', worst: 'O(n²)' },
    })

    steps.push({
      concept: {
        type: 'bigO',
        curves: makeCurves(['O(n)', 'O(n log n)', 'O(n²)'], 'O(n²)'),
        maxN: 50,
        yLabel: 'guaranteed max cost',
      },
      description: d(
        locale,
        'Worst-case bounds are guarantees: even if the input is adversarial, the algorithm will not exceed this growth class.',
        'Los límites de peor caso son garantías: incluso si la entrada es adversarial, el algoritmo no excederá esta clase de crecimiento.',
      ),
      codeLine: 14,
      variables: { guarantee: 'maximum cost over all inputs', use: 'safety and reliability' },
    })
...
This is the foundation of Dynamic Programming (bottom-up).`,
  cppCode: getCppCode('memoization'),
...
def dp_coin_change(coins, amount):
    # Start each amount as impossible
    ...
    for i in range(1, amount + 1):
        for coin in coins:
            if coin <= i and dp[i - coin] + 1 < dp[i]:
                dp[i] = dp[i - coin] + 1
                used[i] = coin
    # Rebuild answer by following used coins
    result = []
    rem = amount
    while rem > 0:
...
Greedy fails here because picking the largest coin first doesn't guarantee the minimum number of coins.`,
  cppCode: getCppCode('greedy-vs-dp'),
