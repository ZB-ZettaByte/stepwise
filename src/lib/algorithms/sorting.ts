...
import { d, arrayOrDefault } from '@lib/algorithms/shared'
    ...
    for i in range(n - 1):
        for j in range(n - i - 1):
            if array[j] > array[j + 1]:
                # bigger value bubbles toward the end
...
Mainly used for educational purposes. For production, prefer Quick Sort or Merge Sort.`,
  cppCode: getCppCode('bubble-sort'),

  generateSteps(locale = 'en', input?: AlgorithmInput) {
    const arr = arrayOrDefault(input, [38, 27, 43, 3, 9, 82, 10])
    const steps: Step[] = []
    const sorted: number[] = []
    const n = arr.length
...
Useful when memory writes are expensive, as it performs at most O(n) swaps.`,
  cppCode: getCppCode('selection-sort'),

  generateSteps(locale = 'en', input?: AlgorithmInput) {
    const arr = arrayOrDefault(input, [64, 25, 12, 22, 11, 90, 45])
    const steps: Step[] = []
    const sorted: number[] = []
    const n = arr.length
...
Excellent for small datasets or nearly sorted data. Often used as the base case in hybrid sorting algorithms like Timsort.`,
  cppCode: getCppCode('insertion-sort'),

  generateSteps(locale = 'en', input?: AlgorithmInput) {
    const arr = arrayOrDefault(input, [12, 11, 13, 5, 6, 7, 42])
    const steps: Step[] = []
...
const quickSort: Algorithm = {
  ...
  code: `def quick_sort(arr, low=0, high=None):
    # on first call, high is last index
    if high is None:
        high = len(arr) - 1
...
const mergeSort: Algorithm = {
  ...
  visualization: 'array',
  acceptsCustomArray: true,
  code: `def merge_sort(arr, start=0, end=None):
    # on first call, end is last index
    if end is None:
        end = len(arr) - 1
...
const heapSort: Algorithm = {
  ...
  code: `def heap_sort(array):
    ...
    # pull the largest value out repeatedly
...
const countingSort: Algorithm = {
  ...
  code: `def counting_sort(array):
    ...
    # turn counts into running end positions
    for i in range(1, max_value + 1):
        count[i] += count[i - 1]

    # walk backwards to stay stable
...
Ideal for sorting integers within a known, limited range.`,
  cppCode: getCppCode('counting-sort'),

  generateSteps(locale = 'en', input?: AlgorithmInput) {
    const arr = arrayOrDefault(input, [4, 2, 2, 8, 3, 3, 1])
    const steps: Step[] = []
    const n = arr.length
...
const radixSort: Algorithm = {
  ...
  visualization: 'array',
  acceptsCustomArray: true,
  code: `def radix_sort(array):
    max_value = max(array)

    # process one digit place at a time
    exp = 1
...
Ideal for sorting large sets of integers or strings with bounded length.`,
  cppCode: getCppCode('radix-sort'),

  generateSteps(locale = 'en', input?: AlgorithmInput) {
    const arr = arrayOrDefault(input, [170, 45, 75, 90, 802, 24, 2, 66])
    const steps: Step[] = []
    const n = arr.length
    const max = Math.max(...arr)
...
const bucketSort: Algorithm = {
  ...
  code: `def bucket_sort(array, bucket_size=5):
    if len(array) == 0:
        return array
    ...
    # 2. make one empty bucket per value range
    bucket_count = (high - low) // bucket_size + 1
    buckets = []
    for i in range(bucket_count):
        buckets.append([])

    # 3. drop each value into its bucket
    for i in range(len(array)):
        bucket_index = (array[i] - low) // bucket_size
        buckets[bucket_index].append(array[i])

    # 4. sort each bucket, then join them
    result = []
    for i in range(bucket_count):
...
Ideal for sorting floating-point numbers or data uniformly distributed over a range.`,
  cppCode: getCppCode('bucket-sort'),
