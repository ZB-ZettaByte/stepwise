...
const bfs: Algorithm = {
  ...
  code: `def bfs(graph, start):
    ...
    # Mark start so it is not requeued
    visited.add(start)
...
const dfs: Algorithm = {
  ...
  code: `def dfs(graph, start):
    ...
    # Recursion follows one path before backtracking
    def explore(node):
        visited.add(node)
        result.append(node)
...
const dijkstra: Algorithm = {
  ...
  code: `def dijkstra(graph, start):
    ...
    for i in range(n):
        ...
        # Relax: keep a shorter path through u
        for v, w in graph[u]:
...
const prim: Algorithm = {
  ...
  code: `def prim(graph, start):
    ...
    for i in range(n):
        ...
        # Record cheapest edge to each node
        for v, w in graph[u]:
...
const topologicalSort: Algorithm = {
  ...
  difficulty: 'advanced',
  visualization: 'graph',
  code: `def topological_sort(graph, num_nodes):
...
If the graph has a cycle, a topological ordering is not possible (not all nodes will be included in the result).`,
  cppCode: getCppCode('topological-sort'),
...
const kruskalNodes: GraphNode[] = [
  { id: 0, label: 'A', x: 90, y: 75 },
  { id: 1, label: 'B', x: 250, y: 55 },
  { id: 2, label: 'C', x: 410, y: 85 },
  { id: 3, label: 'D', x: 130, y: 245 },
  { id: 4, label: 'E', x: 320, y: 245 },
]

const kruskalEdges: GraphEdge[] = [
...
const kruskal: Algorithm = {
  ...
  difficulty: 'advanced',
  visualization: 'graph',
  code: `def kruskal(vertices, edges):
    mst = []
    total_weight = 0
    ...
    for edge in edges:
        ...
        # If endpoints are already connected, this edge creates a cycle
...
Kruskal's algorithm builds a minimum spanning tree by repeatedly choosing the cheapest edge that does not create a cycle.

How it works:
1. Sort all edges by weight
2. Start with an empty MST
3. Consider edges from smallest to largest
4. Add an edge only if it connects two different components
5. Stop when the MST has V - 1 edges

Union-Find makes cycle detection fast: if two endpoints already have the same root, adding that edge would form a cycle.

Time Complexity: O(E log E)
  Sorting the edges dominates.
...
Applications:
  ...
  cppCode: getCppCode('kruskal-mst'),
  ...
  generateSteps(locale = 'en') {
    ...
    const sorted = [...kruskalEdges].sort((a, b) => (a.weight ?? 0) - (b.weight ?? 0))
    ...
    steps.push({
      graph: {
        ...
        distances: Object.fromEntries(
          sorted.map((e, i) => [
            i,
            `${kruskalNodes[e.from].label}${kruskalNodes[e.to].label}:${e.weight}`,
          ]),
        ),
      },
      description: d(
        locale,
        'Sort edges by weight. Kruskal will scan from cheapest to most expensive.',
        'Ordenar aristas por peso. Kruskal revisa de menor a mayor costo.',
      ),
      codeLine: 7,
      variables: {
        sorted: sorted
          .map((e) => `${kruskalNodes[e.from].label}${kruskalNodes[e.to].label}:${e.weight}`)
          .join(', '),
      },
    })
    ...
    for (const edge of sorted) {
      const u = edge.from
      const v = edge.to
      const labels = `${kruskalNodes[u].label}-${kruskalNodes[v].label}`
      const ru = find(u)
      const rv = find(v)
      ...
      steps.push({
        graph: {
          nodes: kruskalNodes,
          edges: kruskalEdges,
          visitedNodes: [...new Set(mst.flat())],
          currentNode: u,
          visitedEdges: [...mst],
          currentEdge: [u, v],
          distances: { 0: `total ${total}`, 1: `check ${labels}:${edge.weight}` },
        },
        description: d(
          locale,
          `Consider edge ${labels} with weight ${edge.weight}. Components: ${ru} and ${rv}.`,
          `Considerar arista ${labels} con peso ${edge.weight}. Componentes: ${ru} y ${rv}.`,
        ),
        codeLine: 10,
        variables: { edge: labels, weight: edge.weight ?? 0, root_u: ru, root_v: rv },
      })
      ...
      if (union(u, v)) {
        ...
        steps.push({
          graph: {
            ...
            visitedEdges: [...mst],
            currentEdge: [u, v],
            distances: { 0: `total ${total}`, 1: `accepted ${labels}`, 2: `edges ${mst.length}` },
          },
          description: d(
            locale,
            `Accept ${labels}. It connects two components, so it belongs in the MST.`,
            `Aceptar ${labels}. Conecta dos componentes, así que pertenece al MST.`,
          ),
          codeLine: 14,
          variables: { accepted: labels, total, mst_edges: mst.length },
        })
      } else {
        steps.push({
          graph: {
            ...
            visitedEdges: [...mst],
            currentEdge: [u, v],
            distances: { 0: `total ${total}`, 1: `rejected ${labels}`, 2: 'cycle' },
          },
          description: d(
            locale,
            `Reject ${labels}. Both endpoints are already connected, so this edge would create a cycle.`,
            `Rechazar ${labels}. Ambos extremos ya están conectados, así que crearía un ciclo.`,
          ),
          codeLine: 13,
          variables: { rejected: labels, reason: 'cycle', total },
        })
      }

      if (mst.length === kruskalNodes.length - 1) break
    }
    ...
    steps.push({
      graph: {
        ...
        visitedEdges: [...mst],
        currentEdge: null,
        distances: { 0: `total ${total}`, 1: 'minimum spanning tree' },
      },
      description: d(
        locale,
        `MST complete with ${mst.length} edges and total weight ${total}.`,
        `MST completo con ${mst.length} aristas y peso total ${total}.`,
      ),
      codeLine: 21,
      variables: { total_weight: total, edges: mst.length },
    })
...
export { bfs, dfs, dijkstra, prim, kruskal, topologicalSort }
