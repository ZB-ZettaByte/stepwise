...
export interface RedBlackTreeNodeData {
  value: number
  color: 'red' | 'black'
  state: 'normal' | 'current' | 'new' | 'rotating' | 'recolor' | 'placed'
}

export interface RedBlackTreeState {
  type: 'redBlackTree'
  nodes: (RedBlackTreeNodeData | null)[]
  operation?: string
}

export interface DisjointSetItem {
  value: number
  parent: number
  rank: number
  state: 'normal' | 'find' | 'root' | 'union' | 'compressed'
}

export interface DisjointSetState {
  type: 'disjointSet'
  items: DisjointSetItem[]
  operation?: string
  path?: number[]
}
...
export interface Algorithm {
  ...
  /** When true, the array visualizer shows a custom-input field for this algorithm. */
  acceptsCustomArray?: boolean
  generateSteps: (locale?: string, input?: AlgorithmInput) => Step[]
}
